...
//---------Constructeur---------//
	...
	public Joueur(String strJoueur) throws JoueurException
	{
		...
		/*Constructeur à partir d'une string, utile pour le chargement d'une partie
			string de la forme: "idJoueur|posX|posY|pelle|mousquet|armure|"
			Ex: "2|3|4|1|0|0|"
		...
		*/
		...
		while (compt<6)
		{
			...
			posSuiv=strJoueur.indexOf("|", pos+1);	//on récupère la position du prochain | dans la chaine
			if (posSuiv==-1)
			{
			...
			if(compt==0)
			{
				...
				info=strJoueur.substring(pos, posSuiv);	//on lie la chaine de pos à posSuiv-1
			}
			else
			{
				info=strJoueur.substring(pos+1, posSuiv);	//on lie la chaine de pos+1 à posSuiv-1
			}
	...
	{
		/*Fonction qui voie si il y a un coffre à la position du joueur, et vérifie si il contiend des équipement que le joueur n's pas*/
	...
	{
		//Fonction récupérant les différentes pirates à la portée du joueur, et les combat
		//Retourne true, si le joueur est vivant après le combat, false sinon
		if(nTour==1)
		{
			return true;
		}
			...
			if((this.posX == pPosX-1 && this.posY == pPosY-1) || (this.posX == pPosX && this.posY == pPosY-1) || (this.posX == pPosX+1 && this.posY == pPosY-1) || (this.posX == pPosX-1 && this.posY == pPosY) || (this.posX==pPosX && this.posY==pPosY) || (this.posX == pPosX+1 && this.posY == pPosY) || (this.posX == pPosX-1 && this.posY == pPosY+1) || (this.posX == pPosX && this.posY == pPosY+1) || (this.posX == pPosX+1 && this.posY == pPosY+1))
			{
					...
					else
					{
						...
						if(this.posX==pPosX && this.posY==pPosY)
						{
							...
							else
							{
								...
								System.out.println("Le joueur s'est fait attaqué par un boucanier, mais à survécu");
							}
						}
					}
				}
				else if(pirateCourant instanceof Flibustier)
				{
					...
					else if(this.mousquet==false && this.armure==true)
					{
						...
						System.out.println("Le joueur s'est fait attaqué par un flibustier, mais à survécu");
					}
